...
5. **Webhook -> Kantina API:** Como configuramos a Nuvem e os Webhooks Oficiais, naquele exato microssegundo o Mercado Pago atira na Rota de Notificações da Kantina API: `https://kantina-api-production.up.railway.app/wallets/pix-webhook` com o Identificador Original. 
6. **Kantina API -> Recálculo e Liberação:** Nosso `pix-webhook.controller.ts` recebe essa bomba, intercepta, vai no MP checar se o status daquele boleto realmente foi "approved" e se sim, atualiza a transação para `paid` no banco de dados da escola, incrementa a carteira do aluno ANA em **+ 1000 cents (R$ 10,00 reais BRUTOS)** - (o custo financeiro fica por conta da própria empresa operante da Kantina) e retorna finalizado. Quando o pagador puxa a tela pra atualizar o saldo no celular dele, já era, está lá! Mágica.
